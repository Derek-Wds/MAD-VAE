...
if __name__ == "__main__":
    ...
    ROBUSTNESS_NORM = norms[args.norm]
    ...
    for method in methods:
        ...
        while True:
            try:
                ...
                if not (os.path.isfile(
                        f"intermediates_{method_}{ROBUSTNESS_NORM}/batch_{k}_attack"
                ) and os.path.isfile(
                        f"intermediates_{method_}{ROBUSTNESS_NORM}/batch_{k}_orig")
                        ):
                    break
                next(enum)
                i += 1
        ...
        for i, (data_, label_) in enum:
            ...
            for j in range(LARGE_BATCH // BATCH_SIZE):
                ...
                os.makedirs(f"intermediates_{method_}{ROBUSTNESS_NORM}",
                            exist_ok=True)
                torch.save(
                    intermediates,
                    f"intermediates_{method_}{ROBUSTNESS_NORM}/batch_{k}_attack")
