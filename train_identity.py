...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='id_logs', help='Logs directory')
...
def main():
    ...
    model, optimizer, scheduler = init_models(args)
    
    # tratinig steps
    step = 0
    for epoch in range(1, args.epochs+1):
        print('Epoch: {}'.format(epoch))
        recon_losses, img_losses, kl_losses, datas, adv_datas, outputs, step = \
            train(args, dataloader, model, optimizer, step, epoch)
        ...
        if epoch % 5 == 0:
            torch.save(model.state_dict(), '{}/identity/params_{}.pt'.format(args.model_dir, epoch))
        
    torch.save(model.state_dict(), '{}/identity/params.pt'.format(args.model_dir))
